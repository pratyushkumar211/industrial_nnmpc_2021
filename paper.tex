...
on this buffer dataset after every training epoch. The NNs after
...
samples, and all the NNs are trained for 1500 epochs. For all the NN training,
the size of the buffer dataset was chosen as 5$\%$ of the training dataset. The
transient controller performance index for one validation simulation obtained
with the three structured NNs, satK, and MPC controllers is shown in the center
bottom. The top right plot shows the histogram of online computation times of
the NNs and QP solver. Table \ref{table:cdu} gives a summary of the maximum time
required to train the NNs, number of parameters in each NN, memory footprint of
the NNs, and the $\%$ performance loss metric obtained by the structured NNs,
SS, satK, and SH controllers in the validation simulations. 
...
Future research in the area of approximating MPC control laws using NNs can
focus on developing methods for approximating the target selector QP solution,
systematically handling state constraints, extensions to nonlinear MPC problems,
and establishing conditions for guaranteed closed-loop stability with NN
controllers.
...
\section*{Source code}
The repository containing code for the simulation studies performed for this article is located at: \url{https://github.com/pratyushkumar211/industrial_nnmpc_2021}
