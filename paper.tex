...
the validation simulations, and the offline
...
\cite*{stewart:venkat:rawlings:wright:pannocchia:2010}.
...
\begin{table}[h]
    ...
    \begin{tabular}{|c|c|c|c|c|c|}
        ...
        \overline{u}$)} \\
        \hline
        $F_0$ & (1.5, 2.5) & \unitfrac{kg}{m$^3$} & 
        $Q_r$ & (-500, 500) & \unit{kW} \\ 
        $F_1$ & (0.5, 1.5) & \unitfrac{kg}{m$^3$} & 
        $Q_m$ & (-500, 500) & \unit{kW} \\ 
        $D$ & (29.5, 30.5) & \unitfrac{kg}{m$^3$} & 
        $Q_b$ & (-500, 500) & \unit{kW} \\ 
        \hline
        ...
        ($x_s, u_s, d_s$)} \\
        \hline
        $H_r$ & 163.8 & \unit{m} & $F_0$ & 2 & \unitfrac{kg}{m$^3$} \\ 
        $x_{Ar}$ & 0.40 &  & $F_1$ & 0 & \unitfrac{kg}{m$^3$} \\ 
        $x_{Br}$ & 0.54 &  & $D$ & 1 & \unitfrac{kg}{m$^3$} \\ 
...
\begin{figure*}[!h]
    \includegraphics[page=41,
        width=0.5\textwidth, 
        height=0.3\textheight]{cdu_comparision_plots.pdf} \hfill
    \includegraphics[page=43,
        width=0.5\textwidth, 
        height=0.3\textheight]{cdu_comparision_plots.pdf} \vfill
    \vspace{-0.1in}
...
of the NNs and QP solver in the above validation
...
their corresponding speedups, time required to 
...
at steady states, and the training process improves the 
approximation of the optimal feedback law
by the NN only for the transient states. 
...
degradation by the NNs compared with 
the optimal MPC controller is negligible.
The sampling of the state-space for NN training
for one particular operational scenario 
with the corresponding setpoint and disturbance signals 
is the rule to obtain scalability of the NN controllers. 
...
methods in the literature such as storing 
...
The key features for scalability of the proposed approach 
are the structure of NN and data generation for only the 
typical plant operational scenarios. 
The NN design approach was demonstrated to scale on a
...
Parallel computing was used for the data 
generation and graphical processing units for the NN 
training. After the offline design phase, NNs execute MPC 
...
of NNs were established under bounded approximation errors of the optimal 
feedback law, that provides some theoretical support 
...
The computational facilities purchased with funds from the National Science Foundation (CNS-1725797) and administered by the Center for Scientific Computing (CSC) was used for the simulation 
studies presented in this article. 
The CSC is supported by the California NanoSystems Institute and the Materials Research Science and Engineering Center 
