...
%style bibliographic references
%%
%% $Id: elsarticle-template-1-num.tex 149 2009-10-08 05:01:15Z rishi $ % $URL:
%http://lenova.river-valley.com/svn/elsbst/trunk/elsarticle-template-1-num.tex $
%%
...
property of NNs in Section \ref{sec:robustness}. Then, we present simulation
studies on two large process control examples in Section
\ref{sec:simulation_studies} and conclude at the end of this article.
...
equivalent to the difference in outputs of two standard NNs with inputs ($x,
x_s, u_s$) and ($x_s, x_s, u_s$) respectively, with $u_s$ added to the final
output. The software tensorflow \citep{abadi:agarwal:barham:brevdo:et-al:2015}
is used for NN training purposes in this article, and the structured
architecture can be built with symbolic differentiation tools available in
tensorflow. When a rate-of-change penalty on the control is included in the
regulator QP, the input to the structured NN is modified to 
\begin{align*}
    z_0 &= 
    \begin{bmatrix}
...
the size of disturbances for which the NN controller is robust. This result also
holds for other approximate MPC controllers, which use different parametric
functions to approximate the optimal control law such as polynomials and other
...
\begin{figure}[h]
    ...
    \includegraphics[width=0.5\textwidth,
                     height=0.25\textheight]{cstrs_with_flash.pdf}
...
generated using the optimal MPC controller, b) examining closed-loop performance
metrics in simulations carried out with the linear model used in MPC and the
optimal MPC and NN controllers, similar to the simulations performed in this
section with the plant model. Additional probabilistic validation techniques for
NNs trained using robust MPC solutions for nonlinear models are proposed in
...
GPU that has 32 GB memory. The code for the simulation studies are available at:
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.45\textwidth, height=0.25\textheight]{cdu.pdf}
...
all the NNs are trained for 2000 epochs. For all the NN trainings performed,
...
\begin{figure*}[!h]
    \includegraphics[page=41, width=0.5\textwidth,
        height=0.27\textheight]{cdu_comparision_plots.pdf} \hfill
        \includegraphics[page=43, width=0.5\textwidth,
        height=0.27\textheight]{cdu_comparision_plots.pdf} \vfill
    \vspace{-0.1in}
    \begin{center}\includegraphics[page=42, width=0.5\textwidth,
...
samples and the performance is unacceptable for an industrial application. The
...
systematically handle multivariable systems, process constraints, improvements
in QP solution algorithms, and has guaranteed stability properties. The
additional computing hardware for training in large applications and sampling
the plant operational scenarios are additional complexities required to design
NN controllers. The advantage of NNs is their speed of MPC controller execution
and convenience for deployment on memory constrained hardware. Future research
may be focused to establish conditions for guaranteed closed-loop stability with
NN controllers, systematically handling state constraints, approximating the
target selector QP, and extensions to large, nonlinear MPC applications. 
...
\begin{table}[!h]
    ...
    \begin{tabular}{|c|c|c|c|c|c|}
        ...
        \unit{sec$^{-1}$} \\ 
        $A_m$ & 2 & \unit{m$^2$} & $k^{\star}_2$ & $1.8 \times 10^{-6}$ &
        \unit{sec$^{-1}$} \\ 
        ...
        \overline{u}$)} \\
        \hline
        $F_0$ & (1.5, 2.5) & \unitfrac{kg}{sec} & $Q_r$ & (-500, 500) &
        \unit{kW} \\ 
        $F_1$ & (0.5, 1.5) & \unitfrac{kg}{sec} & $Q_m$ & (-500, 500) &
        \unit{kW} \\ 
        $D$ & (29.5, 30.5) & \unitfrac{kg}{sec} & $Q_b$ & (-500, 500) &
        \unit{kW} \\ 
        \hline
        \multicolumn{6}{ |c| }{Setpoint bounds ($\underline{r}_{sp},
        \overline{r}_{sp}$)} \\
        \hline
        $H_r$ & (158.8, 168.8) & \unit{m} & $T_r$ & (303, 323) & \unit{K} \\ 
        $H_m$ & (169.2, 179.2) & \unit{m} & $T_m$ & (310, 316) & \unit{K} \\ 
        $H_b$ & (2.2, 4.2) & \unit{m} & $T_b$ & (303, 323) & \unit{K} \\ 
        \hline
        ...
        $x_{A1}$ & (0.7, 0.85) & & & & \\ 
        \hline       
        \multicolumn{6}{ |c| }{Steady state used for linearization ($x_s, u_s,
        d_s$)} \\
        \hline
